...
use CleanCode\Arquitetura\Dominio\Aluno\Telefone;
...
class RepositorioDeAlunoComPdo implements RepositorioDeAluno
{
	...
	{
		$sql = '
			SELECT cpf, nome, email, ddd, numero as numero_telefone FROM alunos
	...
	private function mapearAluno(array $dadosAluno):Aluno
	{
		...
		foreach($telefones as $linha) {
			$aluno->adicionarTelefones($linha['ddd'], $linha['numero_telefone']);
		}
	...
	public function buscarTodos(): array
	{
		...
		foreach($listaDadosAlunos as $dadosAluno)
		{
			...
			if (!is_null($dadosAluno['ddd']) && !is_null($dadosAluno['numero_telefone'])) {
